...
public class DeveloperDAO
{
    ...
    public static HashMap<String, Object> getDeveloperByCredentials(String user, String password) {
        ...
        if (connectionBD != null) {
            try {
                String query = "SELECT * FROM developer WHERE developerLogin = ? AND password = ?";
                PreparedStatement preparedStatement = connectionBD.prepareStatement(query);
                ...
                if (developerResult.next()) {
                    ...
                    developer.setDeveloperLogin(developerResult.getString("developerLogin"));
                    developer.setName(developerResult.getString("name"));
                    developer.setLastName(developerResult.getString("lastname"));
                    developer.setSecondLastName(developerResult.getString("secondlastname"));
                    ...
                    answer.put("message", "No se encontró ningún desarrollador con las credenciales proporcionadas.");
                }
            } catch (SQLException ex) {
                answer.put("message", "Error: " + ex.getMessage());
            }
    ...
    public static HashMap<String, Object> checkDeveloper(String user) {
        ...
        if (connectionBD != null) {
            try {
                String query = "SELECT COUNT(*) AS count FROM developer WHERE developerLogin = ?";
                PreparedStatement preparedStatement = connectionBD.prepareStatement(query);
                preparedStatement.setString(1, user);
    ...
    public static HashMap<String, Object> checkDeveloperLogIn(String user, String password) {
        ...
        if (connectionBD != null) {
            ...
            if(exists == 1){
                result = 1;
                try {
                String query = "SELECT COUNT(*) AS count FROM developer WHERE developerLogin = ? AND password = ?";
                PreparedStatement preparedStatement = connectionBD.prepareStatement(query);
                preparedStatement.setString(1, user);
                preparedStatement.setString(2, password);
