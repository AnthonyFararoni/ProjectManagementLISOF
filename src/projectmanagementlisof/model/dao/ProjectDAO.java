...
public class ProjectDAO {
    ...
    public static HashMap<String, Object>  isInMoreProjects(int idManager){
        ...
        if (connectionBD != null) {
            try {
                String query = "SELECT COUNT(idProject) AS projectCount FROM projectmanagerresponsible WHERE idProjectManager = ?";
                ...
                answer.put("message", "Ocurrió un error al conectar con la base de datos: " + ex.getMessage());
            }
        } else {
            answer.put("error", true);
    ...
    public static HashMap<String, Object> getManagerProjects(int idManager) {
        ...
        if (connectionBD != null) {
            try {
                String query = "SELECT project.idProject, project.name, project.status, project.description " +
                               "FROM project " +
                ...
                if (!projects.isEmpty()) {
                    ...
                    answer.put("message", "El gerente no tiene proyectos asignados o los proyectos no están activos.");
                }
            } catch (SQLException ex) {
